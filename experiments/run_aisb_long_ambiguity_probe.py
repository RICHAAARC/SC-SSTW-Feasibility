"""Run CPU-only long-sequence AISB ambiguity-set diagnostics.

This extends the targeted shifted-window ambiguity test to a longer sequence.
Only one public burst neighborhood is made ambiguous so the public ambiguity set
stays small and can be scored exhaustively for owner and wrong keys.
"""
...
from run_aisb_long_sequence_probe import _build_long_sequence, _states_with_public_bursts
from run_aisb_payload_probe import _payload_state


def _long_targeted_ambiguity_case(
    case_index: int,
    *,
    burst_count: int,
    ...
    owner_key = f"owner_long_ambiguity_{burst_count}_{message_space_size}_{case_index}"
    ...
    wrong_keys = [
        f"wrong_long_ambiguity_{burst_count}_{message_space_size}_{case_index}_{index}"
    ...
    ambiguous_ordinal = min(5, len(burst_plan) - 1)
    ambiguous_start, ambiguous_template_id = burst_plan[ambiguous_ordinal]
    if ambiguous_start > 0:
        states[ambiguous_start - 1] = templates[ambiguous_template_id].points[0]

    observations = generate_observations(
        states,
        make_random_channel(151000 + 100 * message_space_size + case_index, relation_count=16, noise_std=noise_std),
        seed=152000 + 100 * message_space_size + case_index,
    )
    if gamma != 0.0:
        observations = _apply_quadratic_mismatch(observations, states, gamma=gamma)

    # Delete the first AISB point in every burst. Only the burst with an
    # injected public-looking pre-sample should become ambiguous.
...
def main() -> None:
    tiers = {
        "bursts_10_gamma_0.0_messages_16_wrong_12": [
            _long_targeted_ambiguity_case(index, burst_count=10, message_space_size=16, wrong_key_count=12, gamma=0.0, noise_std=0.012)
            for index in range(4)
        ],
        "bursts_10_gamma_0.5_messages_16_wrong_12": [
            _long_targeted_ambiguity_case(index, burst_count=10, message_space_size=16, wrong_key_count=12, gamma=0.5, noise_std=0.012)
            for index in range(4)
        ],
    }
    summaries = {name: _summarize(cases) for name, cases in tiers.items()}
    report = {
        "status": "aisb_long_ambiguity_synthetic_only_no_video_no_gpu_no_claim",
        "long_ambiguity_contract": {
            "targeted_shifted_window_cluster_count": 1,
