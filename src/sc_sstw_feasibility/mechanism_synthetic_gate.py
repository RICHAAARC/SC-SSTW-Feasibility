"""Frozen two-dimensional synthetic mechanism Gate.
...
def load_and_validate_config(path: Path) -> dict[str, Any]:
    ...
    if [case["case_index"] for case in cases] != list(range(config["synthetic_gate"]["case_count"])):
        raise ProtocolError("case indices must be complete and ordered")
    keys = config["candidate_keys"]
    if len(keys["wrong_key_ids"]) != config["synthetic_gate"]["wrong_key_count"]:
    ...
    required = set(config["synthetic_gate"]["required_edit_cases"])
...
def _channel(config: dict[str, Any], case_index: int) -> tuple[list[list[float]], list[float], float]:
    rng = _rng(_derived_seed(config, "synthetic.channel", case_index))
    matrix = [[rng.uniform(-1.0, 1.0) for _column in range(2)] for _row in range(2)]
    bias = [rng.uniform(-0.25, 0.25) for _index in range(2)]
...
def _truth_candidates(edited: list[dict[str, Any]], config: dict[str, Any]) -> set[tuple[int, str, tuple[int, ...]]]:
    template_id = config["synthetic_protocol"]["template"]["template_id"]
    truth: set[tuple[int, str, tuple[int, ...]]] = set()
    public_segments = [
        segment["segment_id"]
        for segment in config["synthetic_protocol"]["sequence"]["segments"]
        if segment["kind"] == "public"
    ]
    for segment_id in public_segments:
        positions = [(index, record) for index, record in enumerate(edited) if record["segment_id"] == segment_id]
        retained = {int(record["template_index"]) for _, record in positions}
        missing = tuple(index for index in range(12) if index not in retained)
        if positions and len(missing) <= 2:
            truth.add((positions[0][0], template_id, missing))
    return truth


def _candidate_identity(candidate: BurstCandidate) -> tuple[int, str, tuple[int, ...]]:
    return candidate.start_index, candidate.template_id, tuple(_missing_list(candidate))
...
def run_gate(config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    for case in config["synthetic_protocol"]["case_protocols"]:
        ...
        truth = _truth_candidates(edited, config)
        accepted_identities = {_candidate_identity(candidate) for candidate in frozen_candidates}
        covered = len(truth & accepted_identities)
        truth_numerator += covered
        truth_denominator += len(truth)
        ...
        case_results.append(
            {
                ...
                "false_candidate_count": len(accepted_identities - truth),
